...
# 3. CSS 주입 (버튼 실종 방지 및 레이아웃 고정)
st.markdown("""
    <style>
    @import url('https://cdn.jsdelivr.net/gh/orioncactus/pretendard/dist/web/static/pretendard.css');
    * { font-family: 'Pretendard', sans-serif !important; box-sizing: border-box; }
    html, body, [data-testid="stAppViewContainer"] { background-color: #FFFEF5 !important; overflow: hidden; }
    ...
    /* ★ 컬럼 대신 사용할 버튼 컨테이너 로직 ★ */
    /* Streamlit의 기본 버튼 마진과 반응형 동작을 완전히 덮어씌움 */
    div.row-widget.stButton {
        display: inline-block !important;
        width: 48% !important; /* 가로의 절반을 차지하도록 고정 */
...
if st.session_state.step < len(diagnostics):
    ...
    # ★ 컬럼을 사용하되, CSS에서 block 처리를 무력화하여 무조건 가로 유지 ★
